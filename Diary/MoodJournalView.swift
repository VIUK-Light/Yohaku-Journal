...
struct MoodJournalView: View {
    ...
    private func journalChrome<Content: View>(_ content: Content) -> some View {
        content
            ...
            .sheet(isPresented: $showingArticles) { ArticleListView() }
            .sheet(isPresented: $showingMentalHealthCheck) { MentalHealthCheckView() }
            .sheet(isPresented: $showingAppInfo) { AppInfoView() }
            .confirmationDialog(
                "この記録を削除しますか？",
                isPresented: Binding(
